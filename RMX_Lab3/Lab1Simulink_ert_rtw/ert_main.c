...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) Lab1Simulink_M->Timing.taskTime0;
  Lab1Simulink_step();
...
int main(int argc, char **argv)
{
  ...
  MX_ADC1_Init();
  rtmSetErrorStatus(Lab1Simulink_M, 0);
  ...
  Lab1Simulink_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Lab1Simulink_M->extModeInfo, &rtmGetTFinal
    (Lab1Simulink_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Lab1Simulink_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Lab1Simulink_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
