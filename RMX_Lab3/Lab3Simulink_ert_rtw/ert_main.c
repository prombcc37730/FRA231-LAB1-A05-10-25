...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) Lab3Simulink_M->Timing.taskTime0;
  Lab3Simulink_step();
...
int main(int argc, char **argv)
{
  ...
  MX_TIM4_Init();
  MX_TIM8_Init();
  rtmSetErrorStatus(Lab3Simulink_M, 0);
  ...
  Lab3Simulink_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Lab3Simulink_M->extModeInfo, &rtmGetTFinal
    (Lab3Simulink_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Lab3Simulink_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Lab3Simulink_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
